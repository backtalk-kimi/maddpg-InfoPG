...
class BatchEnv(ABC):
    ...
    @abstractmethod
    def loop(self, user_params: Dict, policies, optimizers: Dict[str, optim.Optimizer], schedulers):
        pass
    ...
    # def k_level_communication(self, agent_id, policy_initial, k_levels):
    #     for k in range(0, k_levels):
    #         output_dist = {}
    #         for agent_id in range(self.args.n_agents):
    #             batched_neighbors = []
    #             for neighbor in range(self.args.n_agents):
    #                 if neighbor != agent_id:
    #                     batched_neighbors.append([policy_initial[neighbor], neighbor)
    #             latent_vector = policies[agent_id].forward(policy_initial[agent_id], 1, batched_neighbors)
    #             output_dist[agent] = latent_vector
    #         policy_initial = output_dist
    #     return policy_initial

    # def k_level_communication(self, policies, policy_initial, k_levels, num_left_batches = 1, left_batches = []):
    #     for k in range(0, k_levels):
    #         output_dist = {}
    #         for agent_ix, agent in enumerate(self.args.):
